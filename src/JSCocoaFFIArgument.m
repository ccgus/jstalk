...
- (void)cleanUp {
    [structureTypeEncoding release];
    [pointerTypeEncoding release];
    [customData release];
...
//            JSObjectRef object = JSValueToObject(ctx, value, nil);
//            void* p = ptr;
...
{
    [self setStructureTypeEncoding:encoding withCustomStorage:nil];
}
...
- (void)setStructureTypeEncoding:(NSString*)encoding withCustomStorage:(void*)storagePtr
{
    ...
    structureType.elements = malloc(sizeof(ffi_type*)*(elementCount+1));    // +1 is trailing nil

    int i = 0;
    for (id type in types)
    {
        char charEncoding = *(char*)[type UTF8String];
        structureType.elements[i++] = ffi_typeForTypeEncoding(charEncoding);
    }
    structureType.elements[elementCount] = nil;
}
...
- (ffi_type*)ffi_type {
    ...
    return ffi_typeForTypeEncoding(typeEncoding);
}
...
{
    if (!typeEncoding)    return    NSLog(@"No type encoding set in %@", self), nil;
...
//        ptr = malloc([JSCocoaFFIArgument sizeOfStructure:structureTypeEncoding] + 16 + 4); 
    ...
    if (size == -1)    return    NSLog(@"Can't handle type %c", typeEncoding), nil;
    if (size >= 0)    
    {
...
//    NSLog(@"JSType=%d encoding=%c self=%x", type, typeEncoding, self);
    ...
    switch  (typeEncoding)
    {
        ...
        case    _C_DBL:
        {
            double number = JSValueToNumber(ctx, value, nil);
...
//            BOOL isJSStruct = NSOrderedSame == [fullTypeEncoding compare:@"{JSValueRefAndContextRef" options:0 range:NSMakeRange(0, sizeof("{JSValueRefAndContextRef")-1)];
            ...
            NSInteger numParsed =    [JSCocoaFFIArgument structureFromJSObjectRef:object inContext:ctx inParentJSValueRef:nil fromCString:(char*)[type UTF8String] fromStorage:&p];
            return    numParsed;
        }
        case    _C_SEL:
        {
...
/*
    ...
    *value MUST be nil to be receive allocated JSValue
...
+ (NSInteger)structureToJSValueRef:(JSValueRef*)value inContext:(JSContextRef)ctx fromCString:(char*)c fromStorage:(void**)ptr initialValues:(JSValueRef*)initialValues initialValueCount:(NSInteger)initialValueCount convertedValueCount:(NSInteger*)convertedValueCount
{
    ...
    for (; *c && closedBracesCount != openedBracesCount; c++)
    {
        ...
        if (*c == '"')
        {
            ...
            JSObjectSetProperty(ctx, jsObject, propertyNameJS, valueJS, 0, nil);
...
//    NSLog(@"%@", structureName);
    ...
    for (; *c && closedBracesCount != openedBracesCount; c++)
    {
        ...
        if (*c == '"')
        {
            ...
            JSValueRef    valueJS = JSObjectGetProperty(ctx, object, propertyNameJS, nil);
            JSStringRelease(propertyNameJS);
//            JSObjectRef objectProperty2 = JSValueToObject(ctx, valueJS, nil);
...
//            NSLog(@"%c %@ %x %x", encoding, propertyName, valueJS, objectProperty2);
            if (encoding == '{')
            {
                ...
                {
                    JSObjectRef objectProperty = JSValueToObject(ctx, valueJS, nil);
                    NSInteger numParsed = [self structureFromJSObjectRef:objectProperty inContext:ctx inParentJSValueRef:nil fromCString:c fromStorage:ptr];
                    c += numParsed;
                }
                else    return    0;
            }
...
//        NSLog(@"unboxed=%@", resultString);
    ...
    {
        double v = JSValueToNumber(ctx, value, nil);
        // Integer
...
//            NSLog(@"double %f", v);
    ...
    JSObjectRef jsObject = JSValueToObject(ctx, value, nil);
    JSTBridgedObject* private = JSObjectGetPrivate(jsObject);
    ...
    if (!private)
    {
        ...
        JSValueRef result = JSObjectCallAsFunction(ctx, fn, nil, 1, (JSValueRef*)&jsObject, nil);
...
*/
    ...
    else    if ([private.type isEqualToString:@"externalJSValueRef"])
    {
        ...
        JSValueRef v = valueOfCallback(ctx, nil, JSValueToObject(ctx, value, nil), 0, nil, nil);
...
+ (BOOL)unboxJSArray:(JSObjectRef)object toObject:(id*)o inContext:(JSContextRef)ctx
{
    ...
    NSUInteger length = JSValueToNumber(ctx, JSObjectGetProperty(ctx, object, lengthJS, nil), &exception);
...
+ (BOOL)unboxJSHash:(JSObjectRef)object toObject:(id*)o inContext:(JSContextRef)ctx
{
    ...
    for (i=0; i<length; i++)
    {
        ...
        if (exception)    return    NO;
        if (![self unboxJSValueRef:jsValue toObject:&value inContext:ctx])    return    NO;
        if (!value)    value = [NSValue valueWithPointer:nil];
