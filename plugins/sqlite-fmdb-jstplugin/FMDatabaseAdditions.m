...
va_list args;                                                        \
va_start(args, query);                                               \
JSTResultSet *resultSet = [self executeQuery:query withArgumentsInArray:0x00 orVAList:args];   \
va_end(args);                                                        \
...
- (JSTResultSet*)getTableSchema:(NSString*)tableName {
    
    //result colums: cid[INTEGER], name,type [STRING], notnull[INTEGER], dflt_value[],pk[INTEGER]
    JSTResultSet *rs = [self executeQuery:[NSString stringWithFormat: @"PRAGMA table_info(%@)", tableName]];
...
- (BOOL)columnExists:(NSString*)tableName columnName:(NSString*)columnName {
    ...
    //get table schema
    JSTResultSet *rs = [self getTableSchema: tableName];
